...
interface Props {
  quoteId: string;
  /** 같은 저자 명언 탭 시 이동(패널 모드면 선택 변경, 스택 모드면 push) */
  onNavigateToQuote: (id: string) => void;
  /** 삭제 완료 후 처리(스택 모드: goBack, 패널 모드: 선택 해제) */
  onAfterDelete?: () => void;
}

/**
 * 명언 상세 콘텐츠. 단독 화면(QuoteDetailScreen)과 태블릿 2-pane 상세 패널에서 공용으로 쓴다.
 */
export function QuoteDetailView({quoteId, onNavigateToQuote, onAfterDelete}: Props) {
        ...
        <View style={styles.section}>
          ...
          {quote.related_quotes.map(rq => (
            <TouchableOpacity
              ...
              onPress={() => onNavigateToQuote(rq.id)}>
